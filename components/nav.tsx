...
const Nav = ({ links, dotStyle = false }: navProps) => {
    ...
    return (
      <nav className=' space-y-1 px-5'>
        {links.map(({ icon, title, active }, index) =>
          <Button
            key={`link-${index}`}
            className='capitalize justify-start hover:bg-transparent hover:text-default-600 w-full'
  ...
  return (
    <nav className=' space-y-1 px-5'>
      ...
      {links.map(({ icon, title, active }, index) =>
        <Button key={`link-${index}`} color='secondary' className='hover:ring-0 hover:ring-transparent justify-start w-full' variant={active ? 'default' : 'ghost'}>
          <Icon
            icon={icon}
